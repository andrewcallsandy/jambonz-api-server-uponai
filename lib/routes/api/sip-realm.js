...
router.post('/:sip_realm', async(req, res) => {
  ...
  logger.info({account_sid, sip_realm, hasAdminScope: req.user.hasScope('admin'), params: req.params, originalUrl: req.originalUrl}, 'Processing SIP realm request');
  try {
    ...
    const [existing] = await promisePool.execute('SELECT account_sid FROM accounts WHERE account_sid = ?', [account_sid]);
