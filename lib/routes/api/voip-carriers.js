...
const { getHomerApiKey, getHomerPcap } = require('../../utils/homer-utils');
...
const validateRegistrationFields = (body, existing = null) => {
  ...
  // Allow any non-empty string without whitespace or sip: prefix — single-label hostnames are valid From domains
  if (body.register_from_domain !== undefined &&
      body.register_from_domain !== null &&
      body.register_from_domain !== '') {
    const d = body.register_from_domain;
    if (d !== d.trim() || /\s/.test(d) || /^sips?:/i.test(d)) {
      throw new DbErrorBadRequest(
        'register_from_domain must not contain whitespace or a sip: prefix'
      );
    }
  }

  // Validate register_expires (if provided)
  if (body.register_expires !== undefined && body.register_expires !== null) {
    const expires = parseInt(body.register_expires);
    if (isNaN(expires) || expires <= 0) {
      throw new DbErrorBadRequest('register_expires must be a positive integer (seconds)');
    }
  }
};
...
/**
 * Service-provider carriers listed in JAMBONES_PROTECTED_CARRIER_NAMES (comma-separated,
 * default: Retell) cannot be deleted from the web UI or a normal API DELETE.
 * Ops override: DELETE with header X-Protected-Carrier-Delete-Token matching
 * JAMBONES_PROTECTED_CARRIER_DELETE_TOKEN. Without that env token configured, delete is blocked entirely.
 */
const isProtectedCarrier = (carrier) => {
  if (!carrier || carrier.account_sid) return false; // only SP-scoped (shared) carriers
  const names = (process.env.JAMBONES_PROTECTED_CARRIER_NAMES || 'Retell')
    .split(',')
    .map((n) => n.trim().toLowerCase())
    .filter(Boolean);
  return names.includes(String(carrier.name || '').trim().toLowerCase());
};

const hasProtectedCarrierDeleteOverride = (req) => {
  const expected = process.env.JAMBONES_PROTECTED_CARRIER_DELETE_TOKEN;
  if (!expected) return false;
  const provided = req.get('X-Protected-Carrier-Delete-Token');
...
const validateDelete = async(req, sid) => {
  ...
  if (isProtectedCarrier(carrier) && !hasProtectedCarrierDeleteOverride(req)) {
    throw new DbErrorUnprocessableRequest(
      `cannot delete protected service-provider carrier '${carrier.name}'`
    );
  }
...
/**
 * GET /VoipCarriers/:voip_carrier_sid/pcap/by-carrier-sid
 * Fetches the pcap for a registration-based carrier using the SIP Call-ID
 * stored in register_status.callId
 */
router.get('/:voip_carrier_sid/pcap/by-carrier-sid', async(req, res) => {
  const {logger} = req.app.locals;
  const {voip_carrier_sid} = req.params;
  try {
    ...
    if (!carrier.register_status) {
      return res.status(404).json({msg: 'No registration status found for this carrier'});
    }
    let registerStatus;
    try {
      registerStatus = typeof carrier.register_status === 'string'
        ? JSON.parse(carrier.register_status)
        : carrier.register_status;
    } catch (e) {
      return res.status(500).json({msg: 'Failed to parse register_status'});
    }
    const callId = registerStatus.callId;
    if (!callId) {
      return res.status(404).json({msg: 'No SIP Call-ID found in register_status'});
    }
    const token = await getHomerApiKey(logger);
    if (!token) {
      return res.status(400).json({msg: 'Failed to get Homer API token; check server config'});
    }
    const stream = await getHomerPcap(logger, token, [callId], 'register');
    if (!stream) {
      return res.status(404).json({msg: `No pcap available for carrier ${voip_carrier_sid} (callId: ${callId})`});
    }
    res.set({
      'Content-Type': 'application/octet-stream',
      'Content-Disposition': `attachment; filename=carrier-${voip_carrier_sid}.pcap`
    });
    stream.pipe(res);
  } catch (err) {
    logger.error({err}, `error retrieving pcap for carrier ${voip_carrier_sid}`);
    sysError(logger, res, err);
  }
});
