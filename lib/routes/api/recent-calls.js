...
router.get('/:call_id/pcap', async(req, res) => {
  ...
  try {
    const token = await getHomerApiKey(logger);
    if (!token) return res.sendStatus(400, {msg: 'getHomerApiKey: Failed to get Homer API token; check server config'});
    const method = req.query.method || 'invite';
    const stream = await getHomerPcap(logger, token, [req.params.call_id], method);
    if (!stream) {
      logger.info(`getHomerApiKey: unable to get sip traces from Homer for ${req.params.call_id}`);
      return res.sendStatus(404);
    }
...
router.get('/:call_sid/pcap/by-call-sid', async(req, res) => {
  ...
  try {
    ...
    const {sip_callid} = data.data[0];
    if (!sip_callid) {
      return res.status(404).send({msg: 'No SIP call ID found for this call'});
    }
    const token = await getHomerApiKey(logger);
    if (!token) return res.sendStatus(400, {msg: 'Failed to get Homer API token; check server config'});
    const stream = await getHomerPcap(logger, token, [sip_callid], method);
    if (!stream) {
      logger.info(`unable to get pcap from Homer for call_sid ${call_sid} (sip_callid: ${sip_callid})`);
      return res.sendStatus(404);
    }
    res.set({
      'Content-Type': 'application/octet-stream',
      'Content-Disposition': `attachment; filename=callid-${sip_callid}.pcap`
    });
    stream.pipe(res);
  } catch (err) {
    logger.error({err}, `error retrieving pcap for call_sid ${call_sid}`);
    res.sendStatus(err.statusCode || 500);
  }
});

router.get('/:call_sid/related-call-sids', async(req, res) => {
  const {logger, queryCdrs} = req.app.locals;
  const {call_sid} = req.params;
  const account_sid = parseAccountSid(req);
  if (!account_sid) {
    return res.status(400).json({msg: 'account_sid is required'});
  }

  try {
    // Step 1: Fetch the requested call by call_sid
    const data = await queryCdrs({account_sid, filter: call_sid, page: 1, page_size: 10});
    if (!data || data.data.length === 0) {
      return res.status(404).json({msg: 'Call not found'});
    }
    const requestedCall = data.data[0];
    const {sip_callid, sip_parent_callid, attempted_at} = requestedCall;

    // Step 2: The root SIP Call-ID is the parent's sip_callid.
    // If this call has sip_parent_callid set, it is a child and the parent's sip_callid is that value.
    // Otherwise this call is itself the parent.
    const rootSipCallid = (sip_parent_callid && sip_parent_callid !== '') ? sip_parent_callid : sip_callid;

    logger.info({
      requested_call_sid: call_sid,
      sip_callid,
      sip_parent_callid: sip_parent_callid || null,
      rootSipCallid,
    }, 'related-call-sids: resolving related legs');

    // Step 3: Time-windowed scan — both legs of a transfer are within seconds of each other,
    // but we use a generous ±2 hour window to be safe.
    const baseTime = new Date(attempted_at).getTime();
    const windowStart = new Date(baseTime - 2 * 60 * 60 * 1000).toISOString();
    const windowEnd   = new Date(baseTime + 2 * 60 * 60 * 1000).toISOString();

    const windowData = await queryCdrs({
      account_sid,
      page: 1,
      page_size: 500,
      start: windowStart,
    ...
    const allCalls = (windowData && windowData.data) ? windowData.data : [];

    // Step 4: Keep only legs belonging to the same transfer group.
    // A leg belongs if its own sip_callid is the root, or its sip_parent_callid points to the root.
    const relatedCalls = allCalls.filter((c) =>
      c.sip_callid === rootSipCallid ||
      (c.sip_parent_callid && c.sip_parent_callid === rootSipCallid)
    );

    // Step 5: Build the response legs, classifying parent vs child.
    const legs = relatedCalls.map((c) => ({
      call_sid:         c.call_sid,
      role:             (!c.sip_parent_callid || c.sip_parent_callid === '') ? 'parent' : 'child',
      sip_callid:       c.sip_callid,
      sip_parent_callid: c.sip_parent_callid || null,
      ...
      terminated_at:    c.terminated_at || null,
    }));

    // Parent first, then children sorted by start time
    legs.sort((a, b) => {
      if (a.role !== b.role) return a.role === 'parent' ? -1 : 1;
      return new Date(a.attempted_at) - new Date(b.attempted_at);
    });

    const parentLeg = legs.find((l) => l.role === 'parent');

    logger.info({
      requested_call_sid:       call_sid,
      resolved_parent_call_sid: parentLeg ? parentLeg.call_sid : null,
      total_legs:               legs.length,
    }, 'related-call-sids: resolved');

    return res.status(200).json({
      account_sid,
      requested_call_sid: call_sid,
      primary_call_sid:   parentLeg ? parentLeg.call_sid : call_sid,
      total_legs:         legs.length,
      legs,
    });

  } catch (err) {
    logger.error({err}, `error resolving related call sids for ${call_sid}`);
    res.sendStatus(err.statusCode || 500);
  }
});
