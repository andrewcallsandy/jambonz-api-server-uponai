...
const {deleteDnsRecords, provisionSipRealmDns} = require('../../utils/dns-utils');
...
router.use('/:sid/BlfConfigurations', hasAccountPermissions, require('./blf-configurations'));
...
function validateUpdateCall(opts) {
  ...
  if (opts.listen) {
    if (!['start', 'stop'].includes(opts.listen.action)) {
      throw new DbErrorBadRequest('listen requires action property of \'start\' or \'stop\'');
    }
    if (opts.listen.action === 'start' && !opts.listen.url) {
      throw new DbErrorBadRequest('listen requires url property when action is \'start\'');
    }
  }
}
...
function validateLimits(limits) {
  if (!Array.isArray(limits)) {
    throw new DbErrorBadRequest('\'limits\' must be an array when adding an account');
  }
  ...
  for (const limit of limits) {
    if (!limit.category || !validCategories.includes(limit.category)) {
      throw new DbErrorBadRequest(
        `invalid limit category: '${limit.category}'. Must be one of: ${validCategories.join(', ')}`
      );
    }
    if (typeof limit.quantity !== 'number' || limit.quantity < 0) {
      throw new DbErrorBadRequest(
        `invalid limit quantity for category '${limit.category}': must be a non-negative number`
      );
    }
  }
}
...
async function validateAdd(req) {
  ...
  if (req.body.limits) {
    validateLimits(req.body.limits);
  }
}
...
router.post('/', async(req, res) => {
  ...
  try {
    ...
    // extract limits from request body if present
    ...
    // remove limits from obj before passing to Account.make
    delete obj.limits;

    logger.debug(`Attempting to add account ${JSON.stringify(obj)}`);
    const uuid = await Account.make(obj);

    // create account limits if provided
    if (limits && limits.length > 0) {
      logger.debug(`Creating ${limits.length} limit(s) for account ${uuid}`);
      for (const limit of limits) {
        await AccountLimits.make({
          account_sid: uuid,
...
router.put('/:sid', async(req, res) => {
  ...
  try {
    ...
    // remove limits from obj before passing to Account.update
    delete obj.limits;
    ...
    if (Object.keys(obj).length) {
      ...
      // If sip_realm changed, provision DNS records via DME
      if (obj.sip_realm && process.env.JAMBONES_HOSTING) {
        provisionSipRealmDns(logger, sid, obj.sip_realm).catch((err) => {
          logger.error({err, account_sid: sid, sip_realm: obj.sip_realm},
            'PUT /Accounts: failed to provision DNS for sip_realm change');
        });
      }
    ...
    // update account limits if provided
    if (limits && limits.length > 0) {
      logger.debug(`Updating ${limits.length} limit(s) for account ${sid}`);

      // Retrieve existing limits
      const existingLimits = await AccountLimits.retrieve(sid) || [];

      for (const limit of limits) {
        const existing = existingLimits.find((el) => el.category === limit.category);
        if (existing) {
          // Update existing limit
          await AccountLimits.update(existing.account_limits_sid, {
            category: limit.category,
            quantity: limit.quantity
          });
        } else {
